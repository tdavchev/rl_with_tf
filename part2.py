...
def random_model():
    ...
    while random_episodes < 10:
        ...
        if done:
            random_episodes += 1
            print "Rewards for this episode was: ", reward_sum
            reward_sum = 0
            env.reset()
...
# def train():
    ...
    with tf.Session() as sess:
        ...
        # Reset the gradient buffer, we will collect gradients in gradBuffer until
        # we are ready to update our policy network.
        gradBuffer = sess.run(tvars)
        for ix, grad in enumerate(gradBuffer):
            gradBuffer[ix] = grad * 0

        while episode_number <= total_episodes:
            # Rendering the environment slwos things down,
            ...
            drs.append(reward)
            # record reward (has to be done after we call step()
            # to get reward for previous actions)
            ...
            if done:
                ...
                # Get the gradient for this episode,
                # and save it in the gradBuffer
