...
num_episodes = 10000 # How many epsiodes of game environment to train network with.
pre_train_steps = 10000 # How many steps of random actions beofre training begins.
max_epLength = 50 # The max allowed length of our episode.
load_model = False # Wehther to load a saved model.
path = "./dqn_sumry" # The path to save our model to.
...
summary_writer = tf.summary.FileWriter("train_dqn_sumry")
...
with tf.Session() as sess:
    ...
    for i in xrange(num_episodes):
        ...
        if i % 10 == 0 and i != 0:
            ...
            summary = tf.Summary()
            summary.value.add(tag='Perf_sumry/Reward', simple_value=float(mean_reward))
            summary.value.add(tag='Perf_sumry/Length', simple_value=float(mean_length))
            summary.value.add(tag='Perf_sumry/Value', simple_value=float(mean_value))
